...
namespace Core
{
    public class ConnectionCompetitions //разнести классы по папочкам
    {
        ...
        public static ObservableCollection<Competitions> GetCompetition()
        {
            ObservableCollection<Competitions> competition = new ObservableCollection<Competitions>();

            try
            {
                for (int i = 0; i < connection.Query<int>($"SELECT * FROM Competition;").Count(); i++)
                {
                    competition.Add(new Competitions
                    {
