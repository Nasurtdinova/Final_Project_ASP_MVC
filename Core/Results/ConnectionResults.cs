...
namespace Core
{
    public class ConnectionResults
    {
        ...
        public static void RemoveResult(int idCommand, int idCompetition)
        {
            ...
            catch { throw new Exception("Incorrect data!!!"); }
        }
        ...
        public static void UpdateResult(Result result)
        {
            try
            {
                connection.Query($"update ResultCompetition set ResultCompetition.idCompetition=(select Competition.idCompetition from Competition where Competition.Name = '{result.Compet}')," +
                $"ResultCompetition.idCommand=(select Command.idCommand from Command where Command.Name = '{result.Command}'), " +
